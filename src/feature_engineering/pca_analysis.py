...
def visualize_pca_components(df):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(range(1, len(eig_vals) + 1), eig_vals, alpha=0.5, align='center', label='Variância Individual explicada')
    ax.step(range(1, len(cum_var_exp) + 1), cum_var_exp, where='mid', label='Variância Acumulada explicada')
    ax.set_xlabel('Número de Componentes Principais')
